...
using  "SELECT name FROM sqlite_master" or sqlite_sequence??
    sqlite_sequence is  created if at least oneautoincrement field in table

in def getAllTables(self):
    sql = "SELECT name FROM sqlite_sequence"
        --> does not find renamed table, because not in sqlite_sequence
        --> table are in sqlite_sequence if auto incremented primary key is available
...
## Delete user with id 2
    ...
    def createTable(self, schema):
        #--> change following from self.schema to schema
    ...
    def _checkName(self):
        if self.name:
            ...
            if not _path:
                ...
                self.path = _path
            self.database = os.path.join(self.path, self.name)        
        
    def _checkPaths(self):
        print repr(self.path)
        print repr(self.database)
        if os.path.exists(self.database):
            print 'exists', self.database
        else:
            print 'does not exist', self.database
